...
def click_event(event):
    global selected, place_labels, ship, player_fleet, fleet, board, state, enemy_fleet, s, player_move, enemy_sunk
    if state == "attack":
        if "eFleet" in str(event.widget):
            location = (int(str(event.widget)[9:10]), int(str(event.widget)[12:13]))
            if location in player_move:
                Thread(target=text, args=(["Already fired there", ""], 1,)).start()
                return
            player_move.append(location)
            hit = False
            ...
            if hit:
                ...
                z = 0
                for x in enemy_fleet[ship]:
                    for y in player_move:
                        if (x[0], x[1]) == y:
                            z += 1
                if z == fleet[ship]["size"]:
                    Thread(target=text, args=(["You sunk " + ship, ""], 2,)).start()
                    enemy_sunk.append(ship)
                    if len(enemy_sunk) == 7:
                        state = "won"
                        place_labels[0]["text"] = "You won"
    ...
    while True:
        ...
        if state == "placed":
            ...
            if hit:
                ...
                z = 0
                for x in player_fleet[ship]:
                    for y in enemy_move:
                        if (x[0], x[1]) == y:
                            z += 1
                if z == fleet[ship]["size"]:
                    Thread(target=text, args=(["You sunk my " + ship, ""], 2,)).start()
                    player_sunk.append(ship)
                    if len(player_sunk) == 7:
                        state = "lost"
                        place_labels[0]["text"] = "You lost"
            else:
                board[0][location[0]][location[1]]["bg"] = "blue"
            if state == "wait":
                state = "attack"
                place_labels[0]["text"] = "Your turn"
...
if __name__ == '__main__':
    ...
    [
    "Welcome to", "Battleships", "Click", "The", "Buttons", "Below", "To", "Place", "Your", "Fleet", "Push",
    "R", "To","Rotate", ""
     ], 0.5,)).start()
